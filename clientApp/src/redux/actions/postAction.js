import { CREATE_COM, CREATE_POST, GET_COMS, GET_POSTS } from '../types/types';
...
export const CREATE_POST_THUNK = (data, id) => (dispatch) => {
  fetch(
    ...
    if (res) {
      dispatch(createPOST(res.json()));
    }
  });
};

export const GET_COMS_THUNK = (id) => (dispatch) => {
  fetch(`https://js-helper.herokuapp.com/com/${1}`)
    .then((res) => res.json())
